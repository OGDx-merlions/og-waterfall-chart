...
,clientWidth=compStyles.clientWidth==0?960:compStyles.clientWidth,width=!this.notResponsive?parseInt(clientWidth):this.width,height=this.height;var svg=d3.select(this.$.waterSVG);var margin=this.chartMargins;this._setInnerDimensions({width:width-margin.left-margin.right,height:this.height-margin.top-margin.bottom});if(this.yAxisLabel!='')margin.left+=50;this._buildChart(svg)},_processData:function _processData(){if(!this.data||this.data.length==0)return[];var data=[],cumulative=+this.startingValue,currentGrp=this.data[0].groupName;// console.log(this.startingValue);
// data.push({
...
.attr('viewBox','0 0 '+(this.innerDimensions.width+margin.left+margin.right)+' '+(this.innerDimensions.height+margin.top+margin.bottom)).append('g').attr('class','container').attr('transform','translate('+margin.left+','+margin.top+')');var g=svg.select('.container');this._buildAxes(axes,g,x);this._buildBars(data,g,x,y);this._addLegend(svg);// text label for the y axis
...
leg.attr('transform','translate('+(this.innerDimensions.width-(this.legends.length*100-35*this.legends.length))+',5)')},_dataChanged:function _dataChanged(){if(this.isChartBuilt){var svg=d3.select(this.$.waterSVG);svg.select('.container').remove();this._buildChart(svg)}},_resize:function _resize(){if(this.data||this.data.length>0){var svg=d3.select('#waterSVG'),compStyles=d3.select(this.$.chart).node(),width=parseInt(compStyles.clientWidth),height=parseInt(this.height),margin=this.chartMargins;this._setInnerDimensions({width:width-margin.left-margin.right,height:height-margin.top-margin.bottom});svg.select('.container').remove();svg.select('.legend').remove();this._buildChart(svg)}}})})();
