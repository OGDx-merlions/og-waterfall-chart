...
var svg=d3.select('#waterSVG'),compStyles=d3.select('#chart').node(),height=parseInt(this.height),margin=this.chartMargins,width=parseInt(compStyles.clientWidth)+margin.right;// console.log(compStyles.getBoundingClientRect(), compStyles.offsetWidth);
// console.log(d3.select("#chart").node().clientWidth);
if(compStyles.clientWidth>0){this._setInnerDimensions({width:width-margin.left-margin.right,height:height-margin.top-margin.bottom});svg.select('.container').remove();svg.select('.legend').remove();this._buildChart(svg)}},ready:function ready(){this.scopeSubtree(this.$.waterSVG,true)},attached:function attached(){if(!this.notResponsive)d3.select(window).on('resize',this._resize.bind(this));//TODO: remove this and just add a default height and width, let the window resize take care of this.
var compStyles=d3.select(this.$.chart).node()//window.getComputedStyle(d3.select("#chart").node())
,clientWidth=compStyles.clientWidth==0?960:compStyles.clientWidth,width=!this.notResponsive?parseInt(clientWidth):this.width,height=this.height;var svg=d3.select(this.$.waterSVG);var margin=this.chartMargins;this._setInnerDimensions({width:width-margin.left-margin.right,height:this.height-margin.top-margin.bottom});if(this.yAxisLabel!='')margin.left+=30;//if data is just empty then we can't continue building the chart
if(!this.data||this.data.length==0)return false;this._buildChart(svg)},_processData:function _processData(){if(!this.data||this.data.length==0)return[];var data=[],cumulative=+this.startingValue,currentGrp=this.data[0].groupName;// console.log(this.startingValue);
// data.push({
...
//   });
// }
}return data},_xyBounds:function _xyBounds(data){var ceilDiff=+this.ceilingValue-d3.max(data,function(d){return d.end}),intervalMultiplier=Math.ceil(ceilDiff/this.yTickInterval),ceilAdd=intervalMultiplier*this.yTickInterval;var bounds={ceiling:d3.max(data,function(d){return d.end})+(+this.ceilingValue==0?this.yTickInterval:ceilAdd),x:d3.scaleBand().rangeRound([0,this.innerDimensions.width]).paddingInner(0.05),y:d3.scaleLinear().rangeRound([this.innerDimensions.height,0])//x column domain
...
.append('g').attr('class','container').attr('transform','translate('+margin.left+','+margin.top+')');// if(!this.isChartBuilt){
svg.attr('viewBox','0 0 '+(this.innerDimensions.width+margin.left+margin.right)+' '+(this.innerDimensions.height+margin.top+margin.bottom));// .attr("preserveAspectRatio", "xMidYMid meet");
// }
var g=svg.select('.container');this._buildAxes(axes,g,x);this._buildBars(data,g,x,y);this._addLegend(svg);// text label for the y axis
...
leg.attr('transform','translate('+(this.innerDimensions.width-(this.legends.length*100-35*this.legends.length))+',5)')},_dataChanged:function _dataChanged(){if(this.isChartBuilt){var svg=d3.select(this.$.waterSVG);svg.selectAll('*').remove();this._buildChart(svg)}},_resize:function _resize(){if(this.data||this.data.length>0){var svg=d3.select('#waterSVG'),compStyles=d3.select(this.$.chart).node(),width=parseInt(compStyles.clientWidth),height=parseInt(this.height),margin=this.chartMargins;this._setInnerDimensions({width:width-margin.left-margin.right,height:height-margin.top-margin.bottom});svg.select('.container').remove();svg.select('.legend').remove();this._buildChart(svg)}}})})();
