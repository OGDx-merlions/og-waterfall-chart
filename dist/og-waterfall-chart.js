'use strict';(function(){Polymer({is:'og-waterfall-chart',properties:{data:{type:Array,value:[],observer:'_dataChanged'},startingValue:{type:Object,value:{label:'Clean',value:97505}},floorValue:{type:Number,value:96000},yAxisTicks:{type:Number,value:5},yTickInterval:{type:Number,value:500},yAxisLabel:{type:String,value:'LNG mass flow in kg/h'},isChartBuilt:{type:Boolean,value:false,readOnly:true},innerDimensions:{type:Object,readOnly:true,value:{width:960,height:500}},legends:{type:Array,value:[{label:'Increase',color:'#5b9bd5'},{label:'Decrease',color:'#ed7d31'},{label:'Total',color:'#a5a5a5'}]},//chart appearance related properties
width:{type:String,value:'100%'},height:{type:String,value:'300px'},barPadding:{type:Number,value:0.3},barMaxWidth:{type:Number,value:50},chartMargins:{type:Object,value:{top:20,right:30,bottom:100,left:40}},notResponsive:{type:Boolean,value:false}},listeners:{'barClick':'barClick'},barClick:function barClick(d,i){this.fire('bar-click',{data:d,index:i},{bubbles:false})},redrawChart:function redrawChart(){// window.setTimeout(function(){
...
for(var l=0;l<data.length;l++){if(data[l].name==d)return data[l].label}}),yAxis:d3.axisLeft(bounds.y).ticks(this.yAxisTicks).tickValues(d3.range(this.floorValue,bounds.ceiling,this.yTickInterval))};return axes},_buildAxes:function _buildAxes(axes,g){//to display the X axis on the chart
g.append('g').attr('class','x axis').attr('transform','translate(0,'+this.innerDimensions.height+')').call(axes.xAxis).selectAll('text').attr('y',20).attr('x',-20).attr('dy','.35em').attr('transform','rotate(-60)').style('text-anchor','end');//to display the Y axis on the chart
g.append('g').attr('class','y axis').call(axes.yAxis);g.append('g').attr('class','grid').call(axes.yAxis.tickSize(-this.innerDimensions.width).tickFormat(''))},_buildBars:function _buildBars(data,g,x,y){var formatComma=d3.format(',');//adding each bar from the data
...
var barWidth=x.bandwidth()>this.barMaxWidth?this.barMaxWidth:x.bandwidth();bar.append('rect').attr('y',function(d){return y(Math.max(d.start,d.end))}).attr('height',function(d){return Math.abs(y(d.start)-y(d.end))}).attr('width',barWidth);//this is text that appears on the bar
bar.append('text').attr('x',x.bandwidth()/2).attr('y',function(d){return d.class=='decrease'?y(d.end):y(d.end)-10}).attr('dy',function(d){return(d.class=='negative'?'-':'')+'.75em'}).text(function(d){return d.class=='total'?formatComma(d.end):formatComma(d.end-d.start)});bar.filter(function(d){return d.class!='total'}).append('line').attr('class','connector').attr('x1',barWidth).attr('y1',function(d){return y(d.end)}).attr('x2',x.bandwidth()/(1-this.barPadding)-5).attr('y2',function(d){return y(d.end)})},_buildChart:function _buildChart(svg){var margin=this.chartMargins;//transform the data that the start and end properties are added
...
.append('g').attr('class','container').attr('transform','translate('+margin.left+','+margin.top+')');var g=svg.select('.container');this._buildAxes(axes,g);this._buildBars(data,g,x,y);this._addLegend(svg);// text label for the y axis
if(this.yAxisLabel!='')g.append('text').attr('transform','rotate(-90)').attr('y',10-margin.left).attr('x',0-this.height/2).attr('dy','1em').style('text-anchor','middle').text(this.yAxisLabel);this._setIsChartBuilt(true)},_addLegend:function _addLegend(svg){if(!this.legends||this.legends.length==0)return false;var leg=svg.append('g').attr('class','legend');for(var i=0;i<this.legends.length;i++){leg.append('rect').attr('class','leg-rect').attr('x',i*100).attr('y',0).attr('width',10).attr('height',10).style('fill',this.legends[i].color);leg.append('text').attr('x',i*100+12).attr('y',-5).attr('dy','1em').text(this.legends[i].label);// .attr("x", (this.width / 2))
//   .style("text-anchor", "middle");
}leg.attr('transform','translate('+(this.innerDimensions.width/2-(this.legends.length*100+15*this.legends.length)/2)+',0)')},_dataChanged:function _dataChanged(){if(this.isChartBuilt){var svg=d3.select(this.$.waterSVG);svg.select('.container').remove();this._buildChart(svg)}},_resize:function _resize(){var svg=d3.select('#waterSVG'),compStyles=d3.select(this.$.chart).node(),width=parseInt(compStyles.clientWidth),height=parseInt(this.height),margin=this.chartMargins;this._setInnerDimensions({width:width-margin.left-margin.right,height:height-margin.top-margin.bottom});svg.select('.container').remove();svg.select('.legend').remove();this._buildChart(svg)}})})();
