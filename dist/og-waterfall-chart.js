...
,clientWidth=compStyles.clientWidth==0?960:compStyles.clientWidth,width=!this.notResponsive?parseInt(clientWidth):this.width,height=this.height;var svg=d3.select(this.$.waterSVG);var margin=this.chartMargins;this._setInnerDimensions({width:width-margin.left-margin.right,height:this.height-margin.top-margin.bottom});if(this.yAxisLabel!='')margin.left+=30;this._buildChart(svg)},_processData:function _processData(){if(!this.data||this.data.length==0)return[];var data=[],cumulative=+this.startingValue,currentGrp=this.data[0].groupName;// console.log(this.startingValue);
// data.push({
